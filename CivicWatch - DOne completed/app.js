...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initMainMap() {
    ...
    // Map click handler -> continue reporting on the dedicated report page.
    state.map.on("click", (e) => {
      const { lat, lng } = e.latlng;
      window.location.href = `report.html?lat=${encodeURIComponent(lat)}&lng=${encodeURIComponent(lng)}`;
    });
  ...
  function openReportModal() {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        if (!DOM.modalReportOverlay || DOM.modalReportOverlay.classList.contains("open")) {
  ...
  function initReportPage() {
    if (!DOM.formReportIssue || DOM.modalReportOverlay) return;
    const query = new URLSearchParams(window.location.search);
    const lat = Number(query.get("lat"));
    const lng = Number(query.get("lng"));
    const hasCoordinates = Number.isFinite(lat) && Number.isFinite(lng);
    openReportModalWithCoords(hasCoordinates ? lat : undefined, hasCoordinates ? lng : undefined);

    if (hasCoordinates || !navigator.geolocation) return;
    navigator.geolocation.getCurrentPosition(
      (pos) => openReportModalWithCoords(pos.coords.latitude, pos.coords.longitude),
      () => {},
      { timeout: 8000, enableHighAccuracy: true }
    );
  }
  ...
  function setPickedCoordinates(lat, lng) {
    ...
    DOM.reportInputLat.value = lat.toFixed(7);
    DOM.reportInputLng.value = lng.toFixed(7);
    DOM.valPickerLat.textContent = lat.toFixed(7);
    DOM.valPickerLng.textContent = lng.toFixed(7);
    if (state.miniPickerMarker) {
  ...
  function handleDetectGPS() {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        DOM.reportInputAddress.value = `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
        if (DOM.gpsAccuracy) DOM.gpsAccuracy.textContent = `GPS accuracy: about ${Math.round(pos.coords.accuracy)} m`;
  ...
  // --------------------------------------------------------------------------
  async function notifyMunicipalTeam(issue) {
    const settings = APP_CONFIG.EMAILJS;
    if (!settings?.ENABLED) return;
    if (!window.emailjs || !settings.SERVICE_ID || !settings.TEMPLATE_ID || !settings.PUBLIC_KEY) {
      throw new Error("Municipal email is not configured.");
    }
    await window.emailjs.send(settings.SERVICE_ID, settings.TEMPLATE_ID, {
      issue_title: issue.title,
      issue_description: issue.description,
      reporter_name: issue.reporter?.name || "CivicWatch user",
      issue_address: issue.address,
      urgency: issue.severity,
      issue_status: issue.status,
      reported_at: new Date(issue.createdAt).toLocaleString(),
      issue_image: issue.imageUrl
  ...
  async function handleReportSubmit(e) {
    ...
    if (!isAnonymous && !reporterName) {
      showToast("Please enter your name or choose Report Anonymously.", "error");
      DOM.reportInputName.focus();
      return;
    }
    ...
    try {
      ...
      showToast("Civic report submitted successfully! 🚀", "success");
      try {
        await notifyMunicipalTeam(created);
        if (APP_CONFIG.EMAILJS?.ENABLED) showToast("Municipal team notified by email.", "success");
      } catch (emailError) {
        console.warn("Municipal email notification failed:", emailError);
        showToast("Report saved, but the municipal email was not sent.", "info");
      }

      // Reset the form and return to the feed when the report was submitted
      // from its dedicated page. The modal remains supported for map clicks.
      DOM.modalReportOverlay?.classList.remove("open");
      DOM.formReportIssue.reset();
      removeUploadedPhoto();

      if (!DOM.modalReportOverlay) {
        window.location.href = "feed.html?reported=1";
        return;
      }
      ...
      console.error(err);
      showToast(err.message || "Failed to submit civic report.", "error");
    }
  }
  ...
  function showNotifications() {
    showToast("🔔 Issue resolved: Pothole on Main St", "success");
    setTimeout(() => showToast("🚨 New critical hazard: Broken traffic light", "warning"), 1000);
    setTimeout(() => showToast("✅ Your report was verified by city crew", "info"), 2000);
  }
  ...
  function setupEventListeners() {
    ...
    DOM.btnResetDemo?.addEventListener("click", () => {
      if (confirm("Clear all submitted reports?")) {
        ...
        reloadData();
        window.dispatchEvent(new CustomEvent("civicissueschanged"));
